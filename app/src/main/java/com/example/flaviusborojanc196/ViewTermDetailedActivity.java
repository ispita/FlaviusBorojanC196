...
public class ViewTermDetailedActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        final RecyclerView recyclerView = findViewById(R.id.view_courses);
        ...
        FloatingActionButton buttonDeleteTerm = findViewById(R.id.button_delete_term);
        buttonDeleteTerm.setOnClickListener(new View.OnClickListener(){
            @Override
            public void onClick(View v) {
                if (adapterC.getItemCount() == 0) {
                    for (int i = 0; i < adapter.getItemCount(); i++) {
                        Intent intent = new Intent(ViewTermDetailedActivity.this, ViewTermActivity.class);
                        if (Integer.parseInt(viewTextTermId.getText().toString()) == adapter.getTermAt(i).getId()) {
                            termViewModel.delete(adapter.getTermAt(i));
                    ...
                    Toast.makeText(ViewTermDetailedActivity.this, "You CANNOT delete a term that has courses. Please remove all courses and try again.", Toast.LENGTH_SHORT).show();
                }
            }
        });
